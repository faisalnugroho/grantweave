...
LOG = ROOT / 'evidence/live-browser.json'
...
with sync_playwright() as p:
 ...
 def wait_write(label):
  page.wait_for_function("document.querySelector('#tx').textContent.includes('0x') || document.querySelector('#notice').classList.contains('error')",timeout=90000)
  tx_text=page.locator('#tx').inner_text()
